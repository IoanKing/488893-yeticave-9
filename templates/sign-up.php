<main>
  ...
  <form class="form container <?=(!empty($error)) ? 'form--invalid' : ''?>" action="sign-up.php" method="post" autocomplete="off">
    ...
    <div class="form__item <?=isset($error['name']) ? 'form__item--invalid' : ''?>">
      ...
      <input id="name" type="text" name="name" placeholder="Введите имя" value="<?=isset($post['name']) ? esc($post['name']) : ''?>">
    ...
    <div class="form__item <?=isset($error['message']) ? 'form__item--invalid' : ''?>">
      ...
      <textarea id="message" name="message" placeholder="Напишите как с вами связаться"><?=isset($post['message']) ? esc($post['message']) : ''?></textarea>
