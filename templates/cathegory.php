  ...
  <div class="container">
          ...
          <?php foreach ($adverts as $value): ?>
            <li class="lots__item lot">
              ...
              <div class="lot__info">
                ...
                <h3 class="lot__title"><a class="text-link" href="lot.php?id=<?=isset($value['id']) ? esc($value['id']) : ''?>"><?=isset($value['title']) ? esc($value['title']) : ''?></a></h3>
    ...
    <?php if (count($pagination) > 1) : ?>
      <ul class="pagination-list">
        <li class="pagination-item pagination-item-prev"><a href="<?=($page > 1) ? '/cathegory.php?id=' . $cathegory_id . '&page=' . ($page - 1) : ''?>">Назад</a></li>
        <?php if ($pagination && !empty($pagination)): ?>
          <?php foreach ($pagination as $value): ?>
            <li class="pagination-item <?=($page === $value) ? 'pagination-item-active' : ''?>"><a href="<?=($page !== $value) ? '/cathegory.php?id=' . $cathegory_id . '&page=' . $value : ''?>"><?=$value?></a></li>
          <?php endforeach; ?>
        <?php endif;?>
        <li class="pagination-item pagination-item-next"><a href="<?=(($page) < count($pagination)) ? '/cathegory.php?id=' . $cathegory_id . '&page=' . ($page + 1) : ''?>">Вперед</a></li>
      </ul>
    <?php endif; ?>
  </div>
</main>