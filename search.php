<?php
  ...
  $get = $_GET;
  $search_phrase = isset($get['search']) ? esc(trim($get['search'])) : '';
  ...
  $count_adverts = db_fetch_data($DB, isset($query_template['search_count']) ? $query_template['search_count'] : '', $search_phrase);
  ...
  if (isset($count_adverts[0]['count'])) {
    ...
    if ($pages_count > 0) {
      ...
      $adverts = db_fetch_data($DB, isset($query_template['search']) ? $query_template['search'] : '', [$search_phrase, $limit_per_page, $offset]);
