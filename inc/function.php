<?php
    ...
    function get_timer_lelt($date, $is_short = false): string
    {
        ...
        $minutes = (!empty($sign)) ? date_interval_format($time_count, '%I')
          : 0;
            ...
            switch ($key) {
                case 'lot-name':
                    if (empty($post[$key])) {
                        ...
                        if (strlen($post[$key]) > 255) {
                            $errors[$key]
                              = 'Длина наименования лота не должна превышать 255 символов!';
                        }
                    }
                    break;
                case 'category':
                ...
                case 'message':
                    if (empty($post[$key])) {
                        ...
                        if (strlen($post[$key]) > 1200) {
                            $errors[$key]
                              = 'Длина описания лота не должна превышать 1200 символов!';
                        }
                    }
                    break;
                case 'lot-rate':
                ...
                case 'lot-date':
                    if (empty($post[$key])) {
                        ...
                        if (!is_date_valid($post[$key])) {
                            $errors[$key]
                              = 'Дата должна быть в формате ГГГГ-ММ-ДД';
                        } else {
                            if (strtotime($post[$key])
                              <= strtotime(date("Y-m-d"))
                            ) {
                                $errors[$key]
                                  = 'Указанная дата больше текущей даты, хотя бы на один день.';
                            }
                        }
                    }
                    break;
    ...
    function validation_signup($DB, array $post): array
    {
        ...
        if (!isset($post['name']) || empty($post['name'])) {
            ...
            if (strlen($post['name']) > 128) {
                $errors['name']
                  = 'Имя пользователя не должно превышать 128 символов.';
            }
        }
        ...
        if (!isset($post['message']) || empty($post['message'])) {
            ...
            if (strlen($post['password']) > 560) {
                $errors['message']
                  = 'Контасткные сведения не должны превышать 560 символов.';
            }
        }
    ...
    function validation_add_staf($post, array $lot)
    {
        ...
        if (empty($new_staf)) {
            ...
            if (!is_numeric($new_staf)) {
                $errors['staf'] = '2';
            } else {
                if ($new_staf < $current_rate) {
                    $errors['staf'] = '3';
                }
            }
        }
